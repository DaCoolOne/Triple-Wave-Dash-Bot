...
from constants import BALL_RADIUS, DIRECTION, BOOST_CONSUMPTION_RATE, KICKOFF_TYPE, GAME_SPEED
from rlbot.utils.game_state_util import GameState, BallState, CarState, Physics, Vector3, GameInfoState
from rlbot.utils.game_state_util import Rotator as Rotation
...
class Test_Maneuver(Action):
	...
	def update(self, agent, packet):
		...
		if Vec3(1).align_to(my_car.rotation).dot(my_car.location.normal()) < -0.95:
			...
			print(my_car.velocity.length())
			if my_car.velocity.length() <= 0.5:
				print("Setting maneuver")
